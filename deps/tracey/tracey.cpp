...
public:
    check(const char *const text, const char *const file, int line, bool result)
            : xpr(4), ok(result) {
        xpr[0] = std::string(file) + ':' + to_str(line);
        xpr[2] = text;
    }
    ...
    ~check() {
        ...
        if (!ok) {
            ...
            std::string buf;
            buf = "<assert++> says: expression failed! (" + xpr[1] + ") -> (" + xpr[2] + ") -> (unexpected) at " +
                  xpr[0] + "\n";
...
std::string prompt(const std::string &current_value = std::string(), const std::string &title = std::string(),
...
template<typename T>
inline std::string hexdump(const T &obj) {
    return hexdump(obj.data(), obj.size() * sizeof(*obj.begin()), &obj);
}
$cpp11 (
        template<>
        inline std::string hexdump(const std::nullptr_t &obj) {
            return hexdump(0, 0, 0);
        }
)
template<>
inline std::string hexdump(const char &obj) {
    return hexdump(&obj, sizeof(obj), &obj);
}
template<>
inline std::string hexdump(const short &obj) {
    return hexdump(&obj, sizeof(obj), &obj);
}
template<>
inline std::string hexdump(const long &obj) {
    return hexdump(&obj, sizeof(obj), &obj);
}
template<>
inline std::string hexdump(const long long &obj) {
    return hexdump(&obj, sizeof(obj), &obj);
}
template<>
inline std::string hexdump(const unsigned char &obj) {
    return hexdump(&obj, sizeof(obj), &obj);
}
template<>
inline std::string hexdump(const unsigned short &obj) {
    return hexdump(&obj, sizeof(obj), &obj);
}
template<>
inline std::string hexdump(const unsigned long &obj) {
    return hexdump(&obj, sizeof(obj), &obj);
}
template<>
inline std::string hexdump(const unsigned long long &obj) {
    return hexdump(&obj, sizeof(obj), &obj);
}
template<>
inline std::string hexdump(const float &obj) {
    return hexdump(&obj, sizeof(obj), &obj);
}
template<>
inline std::string hexdump(const double &obj) {
    return hexdump(&obj, sizeof(obj), &obj);
}
template<>
inline std::string hexdump(const long double &obj) {
    return hexdump(&obj, sizeof(obj), &obj);
}
/*
...
public:
    ...
    sfstring(const std::string &fmt, const T1 &t1, const T2 &t2, const T3 &t3, const T4 &t4, const T5 &t5)
            : std::string() {
    ...
    sfstring(const std::string &fmt, const T1 &t1, const T2 &t2, const T3 &t3, const T4 &t4, const T5 &t5, const T6 &t6)
            : std::string() {
        std::string t[] = {std::string(), sfstring(t1), sfstring(t2), sfstring(t3), sfstring(t4), sfstring(t5),
                           sfstring(t6)};
        assign(safefmt(fmt, t));
    ...
    sfstring(const std::string &fmt, const T1 &t1, const T2 &t2, const T3 &t3, const T4 &t4, const T5 &t5, const T6 &t6,
             const T7 &t7) : std::string() {
        std::string t[] = {std::string(), sfstring(t1), sfstring(t2), sfstring(t3), sfstring(t4), sfstring(t5),
                           sfstring(t6), sfstring(t7)};
        assign(safefmt(fmt, t));
    }
    ...
    $linux(
            ...
            static std::string sys, tmpfile;
                    ...
                    if (!sys.empty()) {
                        ...
                        sys = sys + (" --tui -q -ex 'set pagination off' -ex 'shell rm " + tmpfile +
                                     "' -ex 'continue' -ex 'finish' -ex 'finish' -ex 'finish' --pid=") + pid +
...
void show(const std::string &body = std::string(), const std::string &head = std::string(),
          const std::string &title = std::string(), bool is_error = false) {
    ...
    $linux(
            if (has("whiptail")) {
                ...
                std::string cmd =
                        std::string("/usr/bin/whiptail --title \"") + headtitle + std::string("\" --msgbox \"") + body +
                        "\" 0 0";
...
std::string demangle(const std::string &mangled) {
    ...
    $linux({
               $no( /* c++filt way */
                       FILE * fp = popen((std::string("echo -n \"") + mangled + std::string("\" | c++filt")).c_str(),
                                         "r");
               ...
               $yes( /* addr2line way. wip & quick proof-of-concept. clean up required. */
                       heal::sfstring
               binary = mangled.substr(0, mangled.find_first_of('('));
    ...
    $windows({
                 char demangled[1024];
                 return (UnDecorateSymbolName(mangled.c_str(), demangled, sizeof(demangled), UNDNAME_COMPLETE))
...
void callstack::save(unsigned frames_to_skip) {
    ...
    $windows({
                 ...
                 typedef WORD(NTAPI
                 FuncRtlCaptureStackBackTrace)(DWORD, DWORD, PVOID *, PDWORD);
                 ...
                 static struct raii {
                     raii() : module(0), ptrRtlCaptureStackBackTrace(0) {
                         ...
                         ptrRtlCaptureStackBackTrace = (FuncRtlCaptureStackBackTrace *) GetProcAddress(module,
                 ...
                 if (module.ptrRtlCaptureStackBackTrace)
                     capturedFrames = module.ptrRtlCaptureStackBackTrace(frames_to_skip + 1, max_frames, out_frames,
                                                                         (DWORD *) 0);
...
std::vector<std::string> callstack::unwind(unsigned from, unsigned to) const {
    ...
    $windows({
                 ...
                 {
                     enum {
                         MAXSYMBOLNAME = 512 - sizeof(IMAGEHLP_SYMBOL64)
                     };
                         ...
                         *symbol64 = *symbol64_blank;
                         ...
                         if (SymGetSymFromAddr64(process, (DWORD64) frames[i], &displacement64, symbol64)) {
                             ...
                             if (SymGetLineFromAddr64(process, (DWORD64) frames[i], &displacement, &line64)) {
                                 backtraces[i] = heal::sfstring("\1 (\2:\3)", symbol64->Name, line64.FileName,
                                                                line64.LineNumber);
                             } else {
...
#   endif
    ...
    result += local::format("%-*s %-.*s [ptr=%p sz=%d]\n", width_offset_block - 1, "offset", width_chars_block - 1,
                            "00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F", self ? self : data, num_bytes);
    ...
    while (i < dumpsize) {
        ...
        for (unsigned b = 0; b < blocks; b++) {
            for (unsigned c = 0; c < width; c++)
                result += local::format(" %c ",
                                        i + c >= dumpsize ? '.' : p[i + c] < 32 || p[i + c] >= 127 ? '.' : p[i + c]);
...
std::string prompt(const std::string &current_value, const std::string &title, const std::string &caption) {
    ...
    if (has("whiptail") && false) {
        std::string out = pipe(std::string() +
                               "/usr/bin/whiptail 3>&1 1>&2 2>&3 --title \"" + title + "\" --inputbox \"" + caption +
                               "\" 0 0 \"" + current_value + "\"");
    }
    else {
...
struct request {
    std::string data;
    /*/      .data 'raw received data, as is' /*/
    std::string method;
    /*/    .method 'raw method ("GET HEAD POST PUT ...")' /*/
    std::string url;
    /*/       .url 'raw request ("/search?req1=blabla&req2=what%20ever")' /*/
    std::string uri;
    /*/      .path 'decoded path uri ("/search")' /*/
    std::string options;
    /*/   .options 'raw GET/POST options url ("req1=blabla&req2=what%20ever")' /*/
    std::map<std::string, std::string> arguments;
    /*/ .arguments 'decoded options decoupled as arguments ( {"req1","blabla"}, {"req2","what ever"} )' /*/
    std::map<std::string, std::string> multipart;
    /*/ .multipart 'multipart files when POST' /*/
    std::string ip;
    /*/        .ip 'ip address of request' /*/
    unsigned port;
    /*/      .port 'number port of request' /*/
    std::string str() const {
...
std::string replace(std::string s, const std::string &target, const std::string &repl) {
    for (size_t it = 0, tlen = target.length(), rlen = repl.length();
         (it = s.find(target, it)) != std::string::npos; it += rlen) {
    ...
    void operator()() {
        ...
        while (!shutdown) {
            ...
            if (hi >= SPACES) {
                ...
                std::string boundary =
                        std::string("--") + extract_from_headers(buf, "Content-Type: multipart/form-data; boundary=");
                ...
                if (found != routers->end()) {
                    ...
                    for (std::vector<std::string>::const_iterator it = split.begin(), end = split.end();
                         it != end; ++it) {
                    ...
                    if (logmask) {
                        char addr[64];
                        sprintf(addr, "%s:%d", rq.ip.c_str(), rq.port);
...
#       if !defined(_WIN32)
        int r = 1;
        setsockopt(socket, SOL_SOCKET, SO_REUSEADDR, &r, sizeof(r));
#       endif
...
/*/
/*/ const httpcode REDIRECT_METHOD = {303,
...
/*/ 5XX server's fault
/*/ const httpcode SERVER_ERROR = {500,
                                   "The server encountered an unexpected condition that prevented it from fulfilling the request."};
/*/
/*/ const httpcode NOT_SUPPORTED = {501,
                                    "The server does not support the functionality required to fulfill the request."};
/*/
/*/ const httpcode BAD_GATEWAY = {502,
...
/*/
/*/ const httpcode VERSION_NOT_SUP = {505,
...
template<typename U>
std::string print(const tree &self, double total, const tracey::string &fmt1234, const tracey::string &sorted_by,
                  bool reversed, const std::map<K, U> &tmap, unsigned depth) {
    ...
    std::string out;
    if (!reversed) {
        for (std::map<std::string, tracey::strings>::const_iterator it = list.begin(), end = list.end();
             it != end; ++it) {
            out += it->second.flat();
        }
    } else {
        for (std::map<std::string, tracey::strings>::const_reverse_iterator it = list.rbegin(), end = list.rend();
             it != end; ++it) {
            out += it->second.flat();
        }
    }
...
template<typename U>
std::string print(const tree &self, double total, const std::map<K, U> &tmap,
                  const tracey::string &format = "{tabs}[{size}] {key} ({value})\n",
...
struct stats_t {
    ...
    std::string str() const {
        return tracey::string(
                "highest peak: \1 total, \2 greatest peak // \3 allocs in use: \4 + overhead: \5 = total: \6",
...
class container
        : public std::map<const void *, leak, std::less<const void *> > //, tracey::malloc_allocator< std::pair< const void *, leak * > > >
{
public:
    ...
    std::string _report() const {
        ...
        kTraceyPrintf("%s",
                      tracey::string("<tracey/tracey.cpp> says: summary: \1" kTraceyCharLinefeed, stats.str()).c_str());
        kTraceyPrintf("%s", tracey::string("<tracey/tracey.cpp> says: creating report: \1" kTraceyCharLinefeed,
                                           logfile).c_str());
        ...
        kTraceyPrintf("%s",
                      tracey::string("<tracey/tracey.cpp> says: found \1 leaks wasting \2" kTraceyCharLinefeed, n_leak,
                                     human(wasted)).c_str());
        ...
        kTraceyfPrintf(fp, "%s", tracey::string("<tracey/tracey.cpp> says: generated with \1 (\2)" kTraceyCharLinefeed,
                                                tracey::version(), tracey::url()).c_str());
        if (tracey::lookup(url) == "????")
            kTraceyfPrintf(fp, "%s", tracey::string(
                    "<tracey/tracey.cpp> says: failed to decode symbols!! Is debug information available?" $msvc(
                            " Are .PDB files available?") kTraceyCharLinefeed).c_str());
        kTraceyfPrintf(fp, "%s", tracey::string(
                "<tracey/tracey.cpp> says: best viewed on foldable text editor (like SublimeText2) with tabs=2sp and no word-wrap" kTraceyCharLinefeed).c_str());
        kTraceyfPrintf(fp, "%s", tracey::string(
                "<tracey/tracey.cpp> says: \1, \2 leaks found; \3 wasted ('\4' score)" kTraceyCharLinefeed,
                !n_leak ? "ok" : "error", n_leak, human(wasted), score).c_str());
        kTraceyfPrintf(fp, "%s", tracey::string("<tracey/tracey.cpp> says: summary: \1" kTraceyCharLinefeed,
                                                stats.str()).c_str());
        kTraceyfPrintf(fp, "%s", tracey::string("<tracey/tracey.cpp> says: report filename: \1" kTraceyCharLinefeed,
                                                logfile).c_str());
        ...
        kTraceyPrintf("%s", tracey::string(
                "<tracey/tracey.cpp> says: creating trees of frames..."  kTraceyCharLinefeed).c_str());
        std::set<void *> set;
        tracey::tree tree;
        ...
        for (leaks::const_iterator it = filtered.begin(), end = filtered.end(); it != end; ++it) {
            ...
            for (unsigned i = 0, start = kTraceyStacktraceSkipBegin, end =
                    callstack.frames.size() - 1 - kTraceyStacktraceSkipEnd; start + i <= end; ++i) {
        ...
        if (!set.size()) {
            if (n_leak) {
                kTraceyPrintf("%s", tracey::string("<tracey/tracey.cpp> says: error! failed to resolve symbols." $msvc(
                        " Are PDB files available?") kTraceyCharLinefeed).c_str());
            }
        } else {
            kTraceyPrintf("%s",
                          tracey::string("<tracey/tracey.cpp> says: resolving \1 unique frames..." kTraceyCharLinefeed,
                                         set.size()).c_str());
            ...
            std::map<void *, std::string> translate;
            {
                if (frames.size() != symbols.size()) {
                    kTraceyfPrintf(fp, "%s", tracey::string(
                            "<tracey/tracey.cpp> says: error! cannot resolve all frames (\1 vs \2)!" kTraceyCharLinefeed,
                            frames.size(), symbols.size()).c_str());
                ...
                kTraceyPrintf("%s", tracey::string(
                        "<tracey/tracey.cpp> says: converting tree of frames into tree of symbols..." kTraceyCharLinefeed).c_str());
                ...
                kTraceyPrintf("%s", tracey::string(
                        "<tracey/tracey.cpp> says: flattering tree of symbols..." kTraceyCharLinefeed).c_str());
                if (kTraceyTruncateBranchesSmallerThan > 0) {
                    tree.walk<tracey::chopper>();
                }

                kTraceyPrintf("%s", tracey::string(
                        "<tracey/tracey.cpp> says: dumping tree log..." kTraceyCharLinefeed).c_str());
                kTraceyfPrintf(fp, "%s",
                               tracey::print(tree, wasted, translate, "{tabs}[{size}] ({value}) {key}\n", "{value}",
                                             true).c_str());
            }
        }
    ...
    if (size == ~0 || size == 0) {
        if (found) {
            ...
            if (kTraceyReportWildPointers)
                kTraceyPrintf("%s", (tracey::string(
                        "<tracey/tracey.cpp> says: Error, wild pointer deallocation." kTraceyCharLinefeed) +
                                     tracey::callstack(true).flat(kTraceyCharTab "\1) \2" kTraceyCharLinefeed,
                                                                  kTraceyStacktraceSkipBegin)).c_str());
        ...
        *log = stats.str();
        ...
        if (found) {
            if (kTraceyReportDoubleAllocations) {
                kTraceyPrintf("%s", (tracey::string(
                        "<tracey/tracey.cpp> says: Error, double pointer allocation. This should never happen" kTraceyCharLinefeed) +
                                     tracey::callstack(true).flat(kTraceyCharTab "\1) \2" kTraceyCharLinefeed,
                                                                  kTraceyStacktraceSkipBegin)).c_str());
            }
...
static std::string settings(const std::string &prefix) {
    ...
    if (tracey::lookup(url) == "????") {
        out += tracey::string("\1failed to decode symbols!! Is debug information available?" $msvc(
                " Are .PDB files available?") kTraceyCharLinefeed, prefix);
    }
    ...
    out += tracey::string("\1with kTraceyBudgetOverhead=\2%" kTraceyCharLinefeed, prefix,
                          (100 + kTraceyBudgetOverhead));
    out += tracey::string("\1with kTraceyMaxStacktraces=\2 range[\3..\4]" kTraceyCharLinefeed, prefix,
                          int(kTraceyMaxStacktraces), int(kTraceyStacktraceSkipBegin), int(kTraceyStacktraceSkipEnd));
    // kTraceyCharLinefeed
    // kTraceyCharTab
    out += tracey::string("\1with kTraceyReportWildPointers=\2" kTraceyCharLinefeed, prefix,
                          kTraceyReportWildPointers ? "yes" : "no");
    out += tracey::string("\1with kTraceyDefineMemoryOperators=\2" kTraceyCharLinefeed, prefix,
                          kTraceyDefineMemoryOperators ? "yes" : "no");
    out += tracey::string("\1with kTraceyMemsetAllocations=\2" kTraceyCharLinefeed, prefix,
                          kTraceyMemsetAllocations ? "yes" : "no");
    out += tracey::string("\1with kTraceyStacktraceSkipBegin=\2" kTraceyCharLinefeed, prefix,
                          int(kTraceyStacktraceSkipBegin));
    out += tracey::string("\1with kTraceyStacktraceSkipEnd=\2" kTraceyCharLinefeed, prefix,
                          int(kTraceyStacktraceSkipEnd));
    out += tracey::string("\1with kTraceyReportOnExit=\2" kTraceyCharLinefeed, prefix,
                          kTraceyReportOnExit ? "yes" : "no");
...
void *calloc(size_t num, size_t size) {
    return kTraceyMemsetAllocations ? tracey::malloc(num * size) : memset(tracey::malloc(num * size), kTraceyMemsetChar,
                                                                          num * size);
}
void *free(void *ptr) {
...
static std::string a(const std::string &text, const std::string &href = "#") {
    return tracey::string("<a href=\"\1\">\2</a>", href, text);
}
        ...
        struct local {
            ...
            int GET_root(route66::request &req, std::ostream &headers, std::ostream &content) {
                ...
                content << get_html_template().
                                                      ...
                                                      replace("{REPORT}",
                                                              a("generate leak report (may take a while)", "report")).
